...
USGS_meta$Site_ID <- as.numeric(as.character(USGS_meta$Site_ID))
...
## Create function to find ComID for multiple sites...
  ...
  beep()
  # If you look at the output dataframe, only the 90 USGS sites that were not in NHD were not assigned a ComID, so we will do those next using a different approach:
  View(USGS_sites)
  ...
  beep()
  # If you look at the output dataframe, there are still 26 sites that were not indexed
  View(USGS_sites)
