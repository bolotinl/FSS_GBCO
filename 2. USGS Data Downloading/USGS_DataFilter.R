...
# Need to filter for HUC 14,15,16
disch_huc_sites <- disch_sensor %>% 
  filter(str_detect(huc_cd, "^14|^15|^16")) %>% 
  ...
  levels(disch_huc_sites$Site_ID) # 2722
SC_huc_sites <- SC_sensor %>% 
  filter(str_detect(huc_cd, "^14|^15|^16")) %>% 
  select(Site_ID,station_nm,Lat,Lon,huc_cd,parm_cd,begin_date,end_date,count_nu,site_tp_cd,data_type_cd,access_cd)
  SC_huc_sites$Site_ID <- factor(SC_huc_sites$Site_ID)  
  levels(SC_huc_sites$Site_ID) # 234

## one method to get sites with both disch and SC:
#   disch_SC_huc_sites <- disch_SC_sensor %>% 
#   filter(str_detect(huc_cd, "^14|^15|^16") & count_nu>=365) %>% 
#   select(Site_ID,station_nm,Lat,Lon,huc_cd,parm_cd,begin_date,end_date,count_nu,site_tp_cd,data_type_cd,access_cd)

## Save dataframes
saveRDS(disch_huc_sites, "USGS_disch_sites.rds") # List of HUC 14, 15, 16 sites with discharge
saveRDS(SC_huc_sites, "USGS_SC_sites.rds") # List of HUC 14, 15, 16 sites with SC
